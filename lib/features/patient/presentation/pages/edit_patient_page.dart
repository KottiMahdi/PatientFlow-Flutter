...
class EditPatientPage extends StatefulWidget {
  ...
  const EditPatientPage({super.key, required this.patientData});
...
// Dropdown selected values
  ...
  void initState() {
    super.initState();
    // Initialize controllers with data from the provided patient snapshopt
    initializeTextControllers();
    // Initialize dropdown selections from the provided patient snapshopt
  ...
  void initializeTextControllers() {
    ...
    prenomController = TextEditingController(text: widget.patientData.prenom);
    ...
    adresseController = TextEditingController(text: widget.patientData.adresse);
    ...
    dernierRdvController =
        TextEditingController(text: widget.patientData.dernierRdv);
    prochainRdvController =
        TextEditingController(text: widget.patientData.prochainRdv);
    paysController = TextEditingController(text: widget.patientData.pays);
    adresseParController =
        TextEditingController(text: widget.patientData.adressePar);
  }
    ...
    if (_formKey.currentState!.validate()) {
      try {
        ...
        PatientEntity patient = PatientEntity(
          ...
          createdAt:
              DateTime.timestamp(), // Preserve the original creation date
        );
        ...
        if (success) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("Error updating patient")),
          );
        }
      } catch (e) {
        debugPrint("Error: $e");
...
// - Extracts all string values from the document and returns them as a list
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title:
            const Text("Edit Patient", style: TextStyle(color: Colors.white)),
